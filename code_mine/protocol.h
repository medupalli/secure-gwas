    ...
    for (int cur = 0; cur < n1; cur++) {
      ...
      while (ikeep[ind] != 1) {
        if (pid > 0) {
          ...
          for (int p = 1; p <= 2; p++) {
            //must swtich and restore each time we generate a new random mat/vec
            mpc.SwitchSeed(10 + p);
            mpc.RandMat(g0_mask, 3, m0);
            mpc.RandVec(miss0_mask, m0);
            mpc.RestoreSeed();
...
bool data_sharing_protocol(MPCEnv& mpc, int pid) {
  ...
  if (pid > 0) {
    //generate same secret shares for DataSharing (SPs) and then give CPs same shares
    mpc.ExportSeed(fs, 0);
  } else {
    for (int p = 1; p <= 2; p++) {
      mpc.ExportSeed(fs, p);
...
bool gwas_protocol(MPCEnv& mpc, int pid) {
  ...
  // Shared variables: data is saved and shared as binary files (so have to read when you read protocol)
  string s;
  fstream fs;
  ofstream ofs;
  ifstream ifs;
  ...
  // Read in SNP list: vectors (individuals)
  Vec<ZZ> snp_pos;
  Init(snp_pos, m0);
  ...
  //USE THIS A LOT: printing out the values you want to
  if (Param::DEBUG) {
    cout << "pheno" << endl;
    mpc.Print(pheno, 5);
    cout << "cov" << endl;
...
//QUALITY CONTROL: output filtered genotypes (SNPs + individuals to keep)
  Vec<ZZ_p> gkeep1;
  Init(gkeep1, m0);
  ...
  if (Param::SKIP_QC) {
    ...
    //when to communicate: dont do computation until you receive bool
    bool history;
    if (pid == 2) {
  ...
  // filters out the SNP positions that are no longer relevant (didnt meet criteria)
